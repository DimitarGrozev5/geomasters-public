...
export const PartnersEkatteForm: React.FC<Props> = ({
  partnerId,
  selectedEkatte,
  invalidates,
  onClose,
  ...
  const { isLoading, mutate: addEmail } = useConfiguredMutation(
    `/api/partners/${partnerId}/firmEKATTE`,
    { method: 'PATCH' },
    invalidates,
    {
      alertOnSuccess: { message: 'Успешно променен адрес на офис' },
      onSuccess: onClose,
    }
  );
...
export default PartnersEkatteForm;
